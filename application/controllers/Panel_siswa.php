...
class Panel_siswa extends CI_Controller {
	...
	public function cetak() {
		...
		$data = array(
			'user'			=> $base_bio,
			'judul_web'		=> "Cetak Bukti Pendaftaran ".ucwords($base_bio->nama_lengkap),
			'thn_ppdb'		=> date('Y', strtotime($base_bio->tgl_siswa)),
			'nilai_rapor'	=> $this->siswa->get_print('study-report', $sess)->rata_rata_nilai / 5,
			'nilai_usbn'	=> $this->siswa->get_print('schtest-val', $sess)->nilai_usbn / 7,
			'nilai_unbk'	=> $this->siswa->get_print('nattest-val', $sess)->nilai_unbk / 4
		);
